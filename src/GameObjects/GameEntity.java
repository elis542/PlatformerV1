...
public abstract class GameEntity {
	...
	private List<String> jumpAnimation = new ArrayList<>();
	...
	public void setJumpAnimation(List<String> animations) {
		jumpAnimation = animations;
	}
	
	public List<String> getJumpAnimation() {
	...
	public boolean landingOn(double directionY) {	
		for (GameEntity a : getLevel().getEntityList()) {
			...
			double deltaX = Math.abs(a.getXPos() - (getXPos()));
			double deltaY = Math.abs((a.getYPos()-a.getEntityHeight()/4) - (getYPos()));

			boolean collisionX = deltaX < (a.getEntityWidth() / 2 + getEntityWidth() / 2);
			boolean collisionY = deltaY < (a.getEntityHeight() / 2 + getEntityHeight() / 2);
			if (collisionX && collisionY && a.getYPos()-20 > (getYPos() + getEntityHeight()/2)) {
				return true;
			}
		} 
		return false;
